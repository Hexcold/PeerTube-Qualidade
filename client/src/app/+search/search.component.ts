...
} from "../shared/shared-video-miniature/video-miniature.component";
...
export class SearchComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subActivatedRoute = this.route.queryParams.subscribe({
      next: (queryParams) => {
        ...
        this.isSearchFilterCollapsed =
          !this.error &&
          (this.isInitialLoad === false ||
            !this.advancedSearch.containsValues());
        this.isInitialLoad = false;
  ...
  search() {
    ...
    forkJoin([
      ...
      next: (results) => {
        ...
        this.pagination.totalItems = results.reduce(
          (p, r) => (p += r.total),
          0
        );
  ...
  removeVideoFromArray(video: Video | VideoChannel | VideoPlaylist) {
    ...
    this.results = this.results.filter(
      (r) => !this.isVideo(r) || r.id !== video.id
    );
  ...
  getLinkType(): LinkType {
    if (this.advancedSearch.searchTarget === "search-index") {
      ...
      if (
        (!this.isUserLoggedIn() && !remoteUriConfig.anonymous) ||
        !remoteUriConfig.users
      ) {
        return "external";
      }
  ...
  private updateUrlFromAdvancedSearch() {
    ...
    this.router.navigate([], {
      ...
      queryParams: Object.assign({}, this.advancedSearch.toUrlObject(), {
        search,
      }),
    });
  }
  ...
  private getVideosObs() {
    const params = {
      ...
      componentPagination: immutableAssign(this.pagination, {
        itemsPerPage: 10,
        itemsRemoved: this.deletedVideos,
      }),
  ...
  private getVideoChannelObs() {
    const params = {
      search: this.currentSearch,
      componentPagination: immutableAssign(this.pagination, {
        itemsPerPage: this.buildChannelsPerPage(),
      }),
      advancedSearch: this.advancedSearch,
    };
  ...
  private getVideoPlaylistObs() {
    const params = {
      search: this.currentSearch,
      componentPagination: immutableAssign(this.pagination, {
        itemsPerPage: this.buildPlaylistsPerPage(),
      }),
      advancedSearch: this.advancedSearch,
    };
  ...
  private getDefaultSearchTarget(): SearchTargetType {
    ...
    if (
      searchIndexConfig.enabled &&
      (searchIndexConfig.isDefaultSearch ||
        searchIndexConfig.disableLocalSearch)
    ) {
      return "search-index";
    }
