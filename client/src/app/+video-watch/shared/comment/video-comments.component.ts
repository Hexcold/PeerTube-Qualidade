...
import { InfiniteScrollerDirective } from "@app/shared/shared-main/common/video-comment-list-admin-owner.component";
...
export class VideoCommentsComponent implements OnInit, OnChanges, OnDestroy {
  ...
  readonly commentHighlightBlock = viewChild<ElementRef>(
  ...
  ngOnInit() {
    this.pluginService.addAction("video-watch-comment-list:load-data", () =>
  ...
  viewReplies(commentId: number, highlightThread = false) {
    ...
    const obs = this.hooks.wrapObsFun(
      this.videoCommentService.getVideoThreadComments.bind(
    ...
    obs.subscribe({
      next: (res) => {
        ...
        if (highlightThread) {
          ...
          setTimeout(
            () => this.commentHighlightBlock().nativeElement.scrollIntoView(),
            0
          );
        }
      },
      ...
      error: (err) => {
        ...
        if (
          highlightThread &&
          errorBody?.code === ServerErrorCode.COMMENT_NOT_ASSOCIATED_TO_VIDEO
        )
          return;
  ...
  async loadMoreThreads(reset = false) {
    ...
    const obs = this.hooks.wrapObsFun(
      this.videoCommentService.getVideoCommentThreads.bind(
        this.videoCommentService
      ),
      params,
      "video-watch",
  ...
  async onWantedToDelete(
    ...
    this.videoCommentService
      .deleteVideoComment(commentToDelete.videoId, commentToDelete.id)
      .subscribe({
        next: () => {
          if (this.highlightedThread?.id === commentToDelete.id) {
            commentToDelete = this.comments.find(
  ...
  async onWantedToRedraft(commentToRedraft: VideoComment) {
    ...
    if (confirm) {
      ...
      const commentToRedraftText = commentToRedraft.text.replace(
        /<br.?\/?>/g,
        "\r\n"
      );
  ...
  onWantToApprove(comment: VideoComment) {
    this.videoCommentService
      .approveComments([{ commentId: comment.id, videoId: comment.videoId }])
      .subscribe({
        next: () => {
  ...
  private resetVideo() {
    ...
    this.syndicationItems =
      this.videoCommentService.getVideoCommentsFeeds(video);
    this.loadMoreThreads();

    if (this.activatedRoute.snapshot.params["threadId"]) {
      this.processHighlightedThread(
        +this.activatedRoute.snapshot.params["threadId"]
      );
    }
  }

  private processHighlightedThread(highlightedThreadId: number) {
    this.highlightedThread = this.comments.find(
      (c) => c.id === highlightedThreadId
    );
