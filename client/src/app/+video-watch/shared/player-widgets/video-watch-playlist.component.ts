...
@Component({
  ...
  styleUrls: [
    "./player-widget.component.scss",
    "./video-watch-playlist.component.scss",
  ],
  imports: [
...
export class VideoWatchPlaylistComponent {
  ...
  constructor() {
    this.userService
      ...
      .subscribe(
        (user) =>
          (this.autoPlayNextVideoPlaylist = user.autoPlayNextVideoPlaylist)
      );

    this.setAutoPlayNextVideoPlaylistSwitchText();

    this.loopPlaylist = getBoolOrDefault(
  ...
  onPlaylistVideosNearOfBottom(position?: number) {
    ...
    if (
      ...
      this.playlistPagination.currentPage * this.playlistPagination.itemsPerPage
    )
      return;

    this.playlistPagination.currentPage += 1;
    this.loadPlaylistElements({
  ...
  loadPlaylistElements(options: {
    ...
    const obs = this.hooks.wrapObsFun(
      ...
      {
        videoPlaylistId: playlist.uuid,
        componentPagination: this.playlistPagination,
      },
      "video-watch",
      ...
      if (playlistElement.video && playlistElement.position >= position) {
        ...
        setTimeout(() => {
          const element = document.querySelector<HTMLElement>(
            ".element-" + this.currentPlaylistPosition
          );
  ...
  navigateToPreviousPlaylistVideo() {
    const previous = this.findPlaylistVideo(
      this.currentPlaylistPosition - 1,
      "previous"
    );
    if (!previous) return;
    ...
    this.router.navigate([], {
      queryParams: { playlistPosition: previous.position, start, stop },
    });
  }
  ...
  navigateToNextPlaylistVideo() {
    ...
    this.router.navigate([], {
      queryParams: { playlistPosition: next.position, start, stop },
    });
  }
