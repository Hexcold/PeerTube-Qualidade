...
export class PluginListInstalledComponent implements OnInit {
  ...
  loadMorePlugins() {
    this.pluginApiService
      .getPlugins(this.pluginType, this.pagination, this.sort)
      .subscribe({
        next: (res) => {
  ...
  isUpdateAvailable(plugin: PeerTubePlugin) {
    return (
      ...
      compareSemVer(plugin.latestVersion, plugin.version) > 0
    );
  }
  ...
  async uninstall(plugin: PeerTubePlugin) {
    ...
    this.pluginApiService.uninstall(plugin.name, plugin.type).subscribe({
      next: () => {
  ...
  async update(plugin: PeerTubePlugin) {
    ...
    this.pluginApiService
      .update(plugin.name, plugin.type)
      .pipe()
      .subscribe({
        next: (res) => {
  ...
  getShowRouterLink(plugin: PeerTubePlugin) {
    return [
      ...
      "plugins",
      "show",
      this.pluginService.nameToNpmName(plugin.name, plugin.type),
    ];
  }
