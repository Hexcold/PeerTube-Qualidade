...
import {
  ...
  ActorAvatarInput,
} from "@app/shared/shared-actor-image/actor-avatar.component";
...
export class VideoOverviewComponent
  implements OnInit, OnDestroy, AfterViewChecked, DisableForReuseHook
{
  ...
  private loadMoreResults() {
    ...
    this.overviewService.getVideosOverview(this.currentPage).subscribe({
      next: (overview) => {
        ...
        for (const value of overview.channels) {
          const channelName = this.server.getHTMLConfig().client.videos
            .miniature.preferAuthorDisplayName
