...
export class VideoAddToPlaylistComponent
  implements OnInit, OnChanges, OnDestroy, DisableForReuseHook {
  ...
  ngOnInit () {
    ...
    this.listenToAccountPlaylistsChangeSub =
  ...
  load () {
    ...
    this.videoPlaylistService
      .listMyPlaylistWithCache(this.user, this.videoPlaylistSearch)
      .subscribe(result => {
        this.playlistsData = result.data
        this.videoPlaylistService.runVideoExistsInPlaylistCheck(this.video().id)
      })
  ...
  toggleOptionalPlaylist (
    ...
    if (element.enabled) {
      ...
      if (!this.isPrimaryCheckboxChecked(playlist)) {
        playlist.optionalRowDisplayed = false
      }
    } else {
      const newElement: PlaylistElement = {
        enabled: true,
        startTimestamp,
        stopTimestamp
      }

      this.addVideoInPlaylist(playlist, newElement)
    }
  ...
  createPlaylist () {
    ...
    const body: VideoPlaylistCreate = {
      displayName,
      privacy: VideoPlaylistPrivacy.PRIVATE
    }

    this.videoPlaylistService.createVideoPlaylist(body)
      .subscribe({
        next: () => {
  ...
  isPrimaryCheckboxChecked (playlist: PlaylistSummary) {
    return this.pendingAddId === playlist.id ||
      playlist.elements.some(e => e.enabled)
  }
  ...
  getOptionalInputName (playlist: PlaylistSummary, element?: PlaylistElement) {
    const suffix = element ? `-${element.playlistElementId}` : ''
  ...
  isPresentMultipleTimes (playlist: PlaylistSummary) {
    return playlist.elements.filter(e => e.enabled).length > 1
  }

  onElementTimestampUpdate (playlist: PlaylistSummary, element: PlaylistElement) {
    if (!element.playlistElementId || !element.enabled) return
    ...
    this.videoPlaylistService
      .updateVideoOfPlaylist(playlist.id, element.playlistElementId, body, this.video().id)
      .subscribe({
        next: () => {
  ...
  private removeVideoFromPlaylist (playlist: PlaylistSummary, elementId: number) {
    this.videoPlaylistService
      .removeVideoFromPlaylist(playlist.id, elementId, this.video().id)
      .subscribe({
        next: () => {
  ...
  private listenToVideoPlaylistChange () {
    ...
    this.listenToVideoPlaylistChangeSub =
      this.videoPlaylistService
        .listenToVideoPlaylistChange(this.video().id)
        .pipe(filter(() => !this.disabled))
        .subscribe(result => this.rebuildPlaylists(result))
  }
  ...
  private rebuildPlaylists (existResult: CachedVideoExistInPlaylist[]) {
    ...
    this.videoPlaylists = this.playlistsData.map(playlist => {
      const existing = existResult.filter(p => p.playlistId === playlist.id)
      ...
      const old = oldPlaylists.find(p => p.id === playlist.id)
      summary.optionalRowDisplayed = old
        ? old.optionalRowDisplayed
        : this.isOptionalRowDisplayed(summary)

      return summary
    })

    debugLogger('Rebuilt playlist state for video %d.', video.id)
    this.cd.markForCheck()
  }
  ...
  private isOptionalRowDisplayed (playlist: PlaylistSummary) {
    ...
    if (elements.length === 1) {
      ...
      return !!(
        ...
        (e.stopTimestamp && e.stopTimestamp !== this.video().duration)
      )
    }
