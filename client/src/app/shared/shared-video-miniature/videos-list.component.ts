...
import {
  GlobalIconComponent,
  GlobalIconName,
} from "@app/shared/shared-icons/global-icon.component";
...
import {
  ...
  GroupDate,
  GroupDateLabels,
} from "../shared-main/date/date-group-label.component";
...
} from "./video-miniature.component";
...
export class VideosListComponent implements OnInit, OnDestroy {
  ...
  readonly getVideosObservableFunction =
    input<
      (
    ...
    >(undefined);
  readonly getSyndicationItemsFunction =
    input<(filters: VideoFilters) => Promise<Syndication[]> | Syndication[]>(
      undefined
    );
  ...
  readonly loadUserVideoPreferences = input(false, {
  ...
  ngOnInit() {
    ...
    const hiddenFilters = this.hideScopeFilter() ? ["scope"] : [];

    this.filters = new VideoFilters(
      this.defaultSort(),
      this.defaultScope(),
      hiddenFilters
    );
  ...
  onNearOfBottom() {
    ...
    // No more results
    if (
      this.lastQueryLength !== undefined &&
      this.lastQueryLength < this.pagination.itemsPerPage
    )
      return;
  ...
  loadMoreVideos(reset = false) {
    ...
    if (reset) {
      ...
      if (
        this.highlightLives() &&
        (!this.filters.live || this.filters.live === "both")
      ) {
    ...
    this.videoRequests.next({
      reset,
      obsVideos: this.getVideosObservableFunction()(
        this.pagination,
        videoFilters ?? this.filters
      ),
  ...
  removeVideoFromArray(video: Video) {
    ...
    this.highlightedLives = this.highlightedLives.filter(
      (v) => v.id !== video.id
    );
  }
  ...
  private updateUrl() {
    ...
    if (
      ...
      this.filters.hasBeenCustomizedByUser()
    ) {
    ...
    this.videoRequests
      .pipe(
        concatMap(({ reset, obsHighlightedLives, obsVideos }) => {
          return forkJoin([obsHighlightedLives, obsVideos]).pipe(
            map(([resHighlightedLives, resVideos]) => ({
              highlightedLives: resHighlightedLives.data,
